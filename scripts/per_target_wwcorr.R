...
for (target in targets) {
    ...
    wcorr_pearson = weightedCorr(stats_target$predicted_score, stats_target$real_score, method="Pearson", weights=stats_target$real_score^2)
    wcorr_spearman = weightedCorr(stats_target$predicted_score, stats_target$real_score, method="Spearman", weights=stats_target$real_score^2)
...
#print("result:")
N = length(targets)
cat(wcorr_pearson_sum/N, N,  file=paste(input_file, ".wwpearson", sep=""), sep="\t")
cat(wcorr_spearman_sum/N, N, file=paste(input_file, ".wwspearman", sep=""), sep="\t")
