...
for (target in targets) {
    ...
    wcorr_pearson = weightedCorr(stats_target$predicted_score, stats_target$real_score, method="Pearson", weights=rank(-stats_target$real_score))
    wcorr_spearman = weightedCorr(stats_target$predicted_score, stats_target$real_score, method="Spearman", weights=rank(-stats_target$real_score))
...
#print("result:")
N = length(targets)
cat(wcorr_pearson_sum/N, N,  file=paste(input_file, ".wrpearson", sep=""), sep="\t")
cat(wcorr_spearman_sum/N, N, file=paste(input_file, ".wrspearman", sep=""), sep="\t")
